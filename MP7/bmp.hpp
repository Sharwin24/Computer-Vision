...
private:
  ...
  void readJPG(const char* filename) {
    // Read JPG file using OpenCV
    cv::Mat image = cv::imread(filename, cv::IMREAD_COLOR);
    if (image.empty()) {
      throw std::runtime_error("Could not open JPG file: " + std::string(filename));
    }
    // Convert to BGR format
    cv::cvtColor(image, image, cv::COLOR_RGB2BGR);
    // Convert to 2D pixel data
    ...
    this->infoHeader.width = image.cols;
    this->infoHeader.height = image.rows;
    this->infoHeader.bit_count = 24; // 24-bit color depth
    this->infoHeader.size_image = image.total() * image.elemSize();
    this->infoHeader.colors_used = 0; // Not used for 24-bit images
    this->infoHeader.compression = 0; // No compression
    this->infoHeader.size = sizeof(BMPInfoHeader);
    this->fileHeader.file_size = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader) + this->infoHeader.size_image;
    this->fileHeader.offset_data = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader);
    this->fileHeader.offset_data = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader);
    this->fileHeader.file_size = this->fileHeader.offset_data + this->infoHeader.size_image;
  }
  ...
  void read(const char* filename) {
    // If the file is a JPG, read it using readJPG
    const std::string ext = std::string(filename).substr(std::string(filename).find_last_of('.'));
    if (ext == ".jpg" || ext == ".jpeg") {
      this->readJPG(filename);
      return;
    }
