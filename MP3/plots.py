...
histograms = ["moon_histogram.csv", "moon_cumulative_histogram.csv"]
histogram_titles = ["Histogram", "Cumulative Histogram"]
...
for i, h in enumerate(histograms):
    ...
    plt.yticks(np.arange(0, np.max(data[:, 1]) * 1.1, np.max(data[:, 1]) / 10))
    plt.gca().set_aspect('auto', adjustable='box')
plt.tight_layout()
plt.savefig("moon_histograms.png", dpi=300)
# plt.show()
...
images = ["moon_equalized.bmp", "moon_lighting_linear.bmp",
          "moon_lighting_quadratic.bmp"]
